...
/*   Updated: 2023/10/03 15:59:40 by lde-sous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	determine_wall(t_data *d)
{
	if (d->r.len_h > d->r.len_v)
	{
		...
		d->r.rayx = d->r.v_rayx;
		d->r.rayy = d->r.v_rayy;
		if (d->dmap[d->r.mapcy][d->r.mapcx] == 'D')
			return ('D');
		else if ((d->r.angle < PI / 2 || d->r.angle > (3 * PI) / 2))
			return ('E');
		else
			return ('W');
	}
